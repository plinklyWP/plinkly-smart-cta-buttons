...
add_action( 'admin_init', function () {
	...
	$opts = [
		...
		'plinkly_default_font_size'   => 'absint',
		'plinkly_default_padding_v'   => 'absint',
		'plinkly_default_padding_h'   => 'absint',
		
		'plinkly_default_logo_size' => 'absint',
		
		'plinkly_enable_ab_test' => 'intval',  
	];
			...
			<div id="plinkly-tabs">
				...
				<div id="tab-basic">
					<?php plinkly_settings_box( [
						...
						[
						    'label' => esc_html__( 'Enable A/B-Testing', 'plinkly-smart-cta-buttons' ),
						    'field' => 'plinkly_enable_ab_test',
						    'type'  => 'checkbox',
						    'desc'  => esc_html__( 'Show A/B-Test controls in the block editor', 'plinkly-smart-cta-buttons' ),
						    ],
					] ); ?>
				</div>
				...
				<div id="tab-style">
					...
					plinkly_settings_box( [
						...
						[ 'label'=>esc_html__('Font Size (px)', 'plinkly-smart-cta-buttons'),  'field'=>'plinkly_default_font_size',  'type'=>'number','min'=>8,'max'=>48 ],
						
						[ 'label'=>esc_html__('Padding Top/Bottom (px)', 'plinkly-smart-cta-buttons'),  'field'=>'plinkly_default_padding_v',  'type'=>'number','min'=>0,'max'=>40 ],
						[ 'label'=>esc_html__('Padding Left/Right (px)', 'plinkly-smart-cta-buttons'),  'field'=>'plinkly_default_padding_h',  'type'=>'number','min'=>0,'max'=>80 ],
						
						[ 'label' => esc_html__('Logo Size (px)', 'plinkly-smart-cta-buttons'), 'field' => 'plinkly_default_logo_size', 'type'  => 'number','min'   => 10,'max'   => 80],
...
function plinkly_settings_box( array $rows ) {
	...
	foreach ( $rows as $row ) {
		...
		if ( isset( $row['label'] ) ) {
			echo ( $row['label'] === wp_strip_all_tags( $row['label'] ) )
				? esc_html( $row['label'] )
			...
			switch ( $type ) {
				...
				case 'checkbox':
					echo '<label><input type="checkbox" name="' . esc_attr( $field ) . '" value="1"' .
					     ...
					     ( isset( $row['desc'] )
					        ? ( $row['desc'] === wp_strip_all_tags( $row['desc'] ) 
					            ? esc_html( $row['desc'] )
